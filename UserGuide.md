...
This guide is for employers using Align to create care profiles, find suitable caregivers, and hand off to agencies.

## 1. Navigate the App

- `Profile` tab: your workspace for care profiles and shortlisted caregivers
- `Search` tab: browse all caregivers or run profile-based matching
...
Align will rank caregivers using structured overlap and language/experience fit.

## 4. Understand Search Results

- Browse mode (`/search` without profile): general caregiver browsing, no match scores
- Matched mode (`/search?profile=<id>`): ranked results with match percentages and fit chips
- Top ranked caregivers appear first with stronger visual emphasis

## 5. Review Caregiver Detail

Open any caregiver card to view:
...
## 6. Shortlist Caregivers

In caregiver detail (matched mode), click the star button to save/remove shortlist.

- Shortlisted caregivers appear on:
- Main `Profile` workspace
- The specific care profile detail page

## 7. Agency Handoff

Use `Agency handoff` from caregiver detail to simulate the referral flow to an agency for hiring/placement follow-up.
